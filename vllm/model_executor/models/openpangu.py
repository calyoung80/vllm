...
class OpenPanguMoE(nn.Module):
    ...
    def __init__(
        ...
        if (hasattr(config, "router_enable_expert_bias")
...
class OpenPanguEmbeddedAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.total_num_heads % tp_size != 0:
            raise ValueError(f"total_num_heads {self.total_num_heads} "
...
class OpenPanguSinkAttention(nn.Module):
    ...
    def __init__(
        ...
        self.param_sink_with_value = getattr(config, "param_sink_with_value",
                                             False)
    ...
    def weight_loader(self, param: nn.Parameter, loaded_weight: torch.Tensor):
        ...
        if is_gguf_weight and isinstance(param, nn.UninitializedParameter):
            ...
            if output_dim is not None:
                ...
                final_shape[
                    output_dim] = final_shape[output_dim] // self.tp_size
    ...
    def forward(
        ...
        output, _ = self.o_proj(attn_output)
        debug_data(output, f"o_proj output")
        return output
...
class OpenPanguDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.use_mla = (hasattr(config, "qk_nope_head_dim")
                        and hasattr(config, "qk_rope_head_dim")
                        and hasattr(config, "v_head_dim")
                        and hasattr(config, "kv_lora_rank"))
        self.use_sink_attention = (hasattr(config, "param_sink_number")
                                   and config.param_sink_number > 0)
        if self.use_mla:
            ...
            attention_bias = getattr(config, "attention_bias",
                                     False) or getattr(config, "bias", False)
        ...
        self.first_k_dense_replace = getattr(config, "first_k_dense_replace",
    ...
    def forward(
        ...
        # Fully Connected
        debug_data(hidden_states, f"layer{self.layer_idx+1} mlp input")
        hidden_states = self.mlp(hidden_states)
        debug_data(hidden_states, f"layer{self.layer_idx+1} mlp output")
        ...
        debug_data(hidden_states, f"layer{self.layer_idx+1} output")
...
class OpenPanguModel(nn.Module):
    ...
    def forward(
        ...
        debug_data(input_ids, "token ids")
        debug_data(positions, "position ids")
        ...
        for param_name, origin_name, shard_id in attn_mlp_replace_mapping:
            if origin_name not in weight_name or (
                ("mlp.experts." in weight_name)
                    and weight_name not in params_dict):
                continue
            ...
            if weight_name.endswith(
                    ".bias") and weight_name not in params_dict:
                continue
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        for name, loaded_weight in weights:
            ...
            if (self.load_attn_mlp_weight(
                    attn_mlp_replace_mapping,
                    params_dict,
                    name,
                    loaded_weight,
                    loaded_params,
            ) or has_experts and self.load_expert_weight(
...
class OpenPanguModelBase(nn.Module, SupportsPP, SupportsLoRA):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.fuse_qkv_a_proj = (hasattr(config, "q_lora_rank")
