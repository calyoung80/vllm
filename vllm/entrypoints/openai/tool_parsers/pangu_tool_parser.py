...
class PanguToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizerBase, enable_reasoning=False):
        ...
        self.pattern = re.escape(self.tool_call_start_token) + "(.*?)" + re.escape(self.tool_call_end_token)
        ...
        if (self.tool_call_start_token_id is None or self.tool_call_end_token_id is None):
            raise RuntimeError("Pangu Tool parser could not locate tool calls start/end " "tokens in the tokenizer!")
        self.is_complete = []
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        ...
        if not (self.tool_call_start_token in model_output and model_output.find(self.tool_call_end_token) != -1):
    ...
    def extract_tool_calls_streaming(
        ...
        if (self.tool_call_end_token in current_text and self.tool_call_end_token not in delta_text):
        ...
        if (self.tool_call_start_token_id in delta_token_ids and len(delta_token_ids) == 1):
            # if it's the only token, return None, so we don't send a chat
            # completion and don't send a control token
            return None
        ...
        try:
            ...
            tool_call_portion = current_text.split(self.tool_call_start_token)[-1].split(self.tool_call_end_token)[0]
            try:
                tool_call_arr: list[dict] = partial_json_parser.loads(tool_call_portion, flags)
                
                self.is_complete.append(is_complete_json(tool_call_portion))
    ...
    def send_current_tool_name_or_none(self, current_tool_call):
        ...
        if self.is_complete[-1] and cur_args_json and not self.streamed_args_for_tool[-1]:
            ...
            delta = DeltaMessage(
                tool_calls=[
                    DeltaToolCall(
                        ...
                        type="function",
                        id=self._get_next_call_id(),
                        function=DeltaFunctionCall(name=function_name, arguments=argument_diff).model_dump(exclude_none=True)
                    ),
                ]
            )
            self.streamed_args_for_tool[self.current_tool_id] += argument_diff
    ...
    def stream_arguments(self, current_tool_call):
        cur_arguments = current_tool_call.get("arguments")
        delta = None
        if (self.is_complete[-1] and not cur_arguments and not self.streamed_args_for_tool[-1]):
            argument_diff = "{}"
            delta = DeltaMessage(
                tool_calls=[
                    DeltaToolCall(
